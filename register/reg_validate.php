	...
	*/
	...
	if($num_rows)
	{
		...
		else
		{
			...
			$account_query=mysqli_query($conn,"INSERT INTO tblaccounts(sid, password, email, lastname, firstname, middlename, date_registered) VALUES('$sid', '$password', '$email', '$lastname', '$firstname', '$mn', '$d')");
			if($account_query)
			{
				//echo "Account successfully created.";
		echo "<script type='text/javascript'> alert('ACCOUNT SUCCESSFULLY CREATED! You may now log-in!'); 
window.location = '../login_template.php' </script>";
	...
	else
	{
		...
		?>
				<script type="text/javascript">
					alert ('YOU CANNOT REGISTER! Only bonafide students and employees of MSEUFCI are allowed!');
					history.back();
				</script>
